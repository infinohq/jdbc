...
import org.infino.jdbc.internal.util.SqlParser;
...
class SqlParserTests {
        ...
        void testQueryParamCount(String sql, int expectedCount) {
                ...
                assertEquals(expectedCount, paramCount,
                                () -> String.format("[%s] returned %d parameters. Expected %d.", sql, paramCount,
                                                expectedCount));
        }
        ...
        private static Stream<Arguments> pameterizedValidSqlProvider() {
                return Stream.of(
                                ...
                                // Many param markers
                                Arguments.of("select X from table where A=? and B=? and C=? and D=? and (E=? or F=?) ",
                                                6),
        ...
        void testUnterminatedCommentQueries(String sql) {
                ...
                IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
                                () -> SqlParser.countParameterMarkers(sql),
                                () -> String.format(
                                                "[%s] did not throw an exception. Expected unterminated comment exception.",
                                                sql));
        ...
        void testUnterminatedStringQueries(String sql) {
                ...
                IllegalArgumentException ex = assertThrows(IllegalArgumentException.class,
                                () -> SqlParser.countParameterMarkers(sql),
                                () -> String.format(
                                                "[%s] did not throw an exception. Expected unterminated string exception.",
                                                sql));
