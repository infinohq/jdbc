...
public class ResultSetMetaDataImpl implements ResultSetMetaData, JdbcWrapper {
    ...
    public boolean isSigned(int column) throws SQLException {
        checkAccessible(column);
        return schema.getColumnMetaData(column - 1).getInfinoType().isSigned();
    }

    @Override
    public int getColumnDisplaySize(int column) throws SQLException {
        checkAccessible(column);
        return schema.getColumnMetaData(column - 1).getInfinoType().getDisplaySize();
    }
    ...
    public int getColumnType(int column) throws SQLException {
        checkAccessible(column);
        return schema.getInfinoType(column - 1).getJdbcType().getVendorTypeNumber();
    }

    @Override
    public String getColumnTypeName(int column) throws SQLException {
        checkAccessible(column);
        return schema.getInfinoType(column - 1).getJdbcType().getName();
    }
    ...
    public String getColumnClassName(int column) throws SQLException {
        checkAccessible(column);
        return schema.getInfinoType(column - 1).getJavaClassName();
    }
