...
import org.infino.jdbc.protocol.http.JsonHttpProtocolFactory;
...
import org.infino.jdbc.transport.http.ApacheHttpTransportFactory;
...
public class ConnectionImpl implements InfinoConnection, JdbcWrapper, LoggingSource {
    ...
    public ConnectionImpl(ConnectionConfig connectionConfig, TransportFactory transportFactory,
        ...
        try {
            ...
            logAndThrowSQLException(
                    log, new SQLNonTransientException(
                            "Could not initialize transport for the connection: " + te.getMessage(), te));
        }
        ...
        if (connectionConfig.getAuthenticationType() != AuthenticationType.AWS_SIGV4_SERVERLESS) {
            try {
                ConnectionResponse connectionResponse = this.protocol
                        .connect(connectionConfig.getLoginTimeout() * 1000);
    ...
    public Statement createStatement(int resultSetType, int resultSetConcurrency, int resultSetHoldability)
            throws SQLException {
        log.debug(() -> logEntry("createStatement (%d, %d, %d)", resultSetType, resultSetConcurrency,
    ...
    public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency,
        ...
        log.debug(() -> logEntry("prepareStatement (%s, %d, %d, %d)", sql, resultSetType, resultSetConcurrency,
                resultSetHoldability));
        checkOpen();
        validateResultSetCharacteristics(resultSetType, resultSetConcurrency, resultSetHoldability);
        PreparedStatement pst = prepareStatementX(sql);
