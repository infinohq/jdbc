...
/**
 * Enum for Infino Data Types.
 * <p>
 * Each type encapsulates mapping to its corresponding JDBC Type and
 * associated properties.
 * <p>
 * Where required, an Infino data type is mapped to the least
 ...
 * </li>
 * <li>
 * Precision of Infino types is based on
 * Infino Reference > Mapping > Field datatypes
 * </li>
 * </ol>
 */
public enum InfinoType {

    // Precision values based on number of decimal digits supported by Java types
    // Display size values based on precision plus additional buffer for visual
    // representation
    ...
    // - scaled_float is internally an Infino long, but treated as Java Double here
    // - ISO8601 representation of DateTime values as yyyy-mm-ddThh:mm:ss.mmmZ ~ 24
    // chars
    ...
    private static final Map<JDBCType, InfinoType> jdbcTypeToInfinoTypeMap;

    static {
        // Map JDBCType to corresponding ElasticsearchType
        jdbcTypeToInfinoTypeMap = new HashMap<>();
        jdbcTypeToInfinoTypeMap.put(JDBCType.NULL, UNDEFINED);
        jdbcTypeToInfinoTypeMap.put(JDBCType.BOOLEAN, BOOLEAN);
        jdbcTypeToInfinoTypeMap.put(JDBCType.TINYINT, BYTE);
        jdbcTypeToInfinoTypeMap.put(JDBCType.SMALLINT, SHORT);
        jdbcTypeToInfinoTypeMap.put(JDBCType.INTEGER, INTEGER);
        jdbcTypeToInfinoTypeMap.put(JDBCType.BIGINT, LONG);
        jdbcTypeToInfinoTypeMap.put(JDBCType.DOUBLE, DOUBLE);
        jdbcTypeToInfinoTypeMap.put(JDBCType.REAL, FLOAT);
        jdbcTypeToInfinoTypeMap.put(JDBCType.FLOAT, DOUBLE);
        jdbcTypeToInfinoTypeMap.put(JDBCType.VARCHAR, KEYWORD);
        jdbcTypeToInfinoTypeMap.put(JDBCType.TIMESTAMP, TIMESTAMP);
        jdbcTypeToInfinoTypeMap.put(JDBCType.TIME, TIME);
        jdbcTypeToInfinoTypeMap.put(JDBCType.DATE, DATE);
        jdbcTypeToInfinoTypeMap.put(JDBCType.VARBINARY, BINARY);
        jdbcTypeToInfinoTypeMap.put(JDBCType.STRUCT, OBJECT);
        jdbcTypeToInfinoTypeMap.put(JDBCType.ARRAY, ARRAY);
    }

    /**
     * Infino designated type name
     */
    ...
    /**
     * Display Size as per
     * {@link java.sql.ResultSetMetaData#getColumnDisplaySize(int)}
     */
    ...
    InfinoType(JDBCType jdbcType, Class<?> javaClass, int precision,
    ...
    public static InfinoType fromJdbcType(JDBCType jdbcType) {
        if (!jdbcTypeToInfinoTypeMap.containsKey(jdbcType)) {
            throw new IllegalArgumentException("Unsupported JDBC type \"" + jdbcType + "\"");
        }
        return jdbcTypeToInfinoTypeMap.get(jdbcType);
    }

    /**
     * Returns the {@link InfinoType} for the specified Infino
     * data type name.
     */
    public static InfinoType fromTypeName(String typeName) {
        return fromTypeName(typeName, false);
    }

    /**
     * Parses a specified Infino type name to determine
     * the corresponding {@link InfinoType}
     *
     * @param typeName           The Infino Type name to parse
     * @param errorOnUnknownType If true, the method throws an
     *                           {@link UnrecognizedInfinoTypeException}
     *                           if the type name specified is not recognized.
     *
     * @return the {@link InfinoType} value corresponding to the
     *         specified type name
     */
    public static InfinoType fromTypeName(String typeName, boolean errorOnUnknownType) {
        try {
            return InfinoType.valueOf(typeName.toUpperCase(Locale.ROOT));
        } catch (IllegalArgumentException iae) {
            if (errorOnUnknownType)
                throw new UnrecognizedInfinoTypeException("Unknown Type: \"" + typeName + "\"", iae);
            else
                return UNSUPPORTED;
        }
    }
