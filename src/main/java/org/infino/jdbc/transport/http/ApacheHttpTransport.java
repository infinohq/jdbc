...
import org.infino.jdbc.transport.http.auth.aws.AWSRequestSigningApacheInterceptor;
...
public class ApacheHttpTransport implements HttpTransport, LoggingSource {
    ...
    public ApacheHttpTransport(ConnectionConfig connectionConfig, Logger log, String userAgent)
        ...
        ApacheHttpClientConnectionFactory connectionFactory = new ApacheHttpClientConnectionFactory(
                new JclLoggerAdapter(log, getSource()));
        ...
        if (connectionConfig.getAuthenticationType() == AuthenticationType.BASIC) {
            ...
            AWSCredentialsProvider provider = connectionConfig.getAwsCredentialsProvider() != null
                    ? connectionConfig.getAwsCredentialsProvider()
    ...
    @Override
    public CloseableHttpResponse doGet(String path, Header[] headers, HttpParam[] params, int timeout)
            throws TransportException {
        return doGet(buildRequestURI(path, params), headers, timeout);
    }

    @Override
    public CloseableHttpResponse doPost(String path, Header[] headers, HttpParam[] params, String body, int timeout)
            throws TransportException {
    ...
    private CloseableHttpResponse doPost(URI uri, Header[] headers, String body, int readTimeout)
            throws TransportException {
        try {
            setReadTimeout(readTimeout);
