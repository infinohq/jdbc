...
import org.infino.jdbc.internal.results.ColumnMetaData;
import org.infino.jdbc.internal.results.Cursor;
import org.infino.jdbc.internal.exceptions.ObjectClosedException;
import org.infino.jdbc.internal.results.Row;
import org.infino.jdbc.internal.results.Schema;
...
import org.infino.jdbc.protocol.http.JdbcCursorQueryRequest;
import org.infino.jdbc.protocol.http.JsonCursorHttpProtocol;
import org.infino.jdbc.protocol.http.JsonCursorHttpProtocolFactory;
...
import org.infino.jdbc.types.UnrecognizedInfinoTypeException;
...
/**
 * Infino implementaion for a {@link ResultSet}
 * <p>
 * Column names or labels received in APIs are treated in a
 * case-sensitive manner since Infino field names are
 * case-sensitive.
 * </p>
 * The lookup
 */
public class ResultSetImpl implements ResultSet, JdbcWrapper, LoggingSource {
    ...
    public ResultSetImpl(StatementImpl statement, QueryResponse queryResponse, Logger log) throws SQLException {
        this(statement, queryResponse.getColumnDescriptors(), queryResponse.getDatarows(), queryResponse.getCursor(),
                log);
    }
    ...
    public ResultSetImpl(StatementImpl statement, List<? extends ColumnDescriptor> columnDescriptors,
        ...
        } catch (UnrecognizedInfinoTypeException ex) {
     ...
     * https://github.com/amazon-archives/sql-jdbc/pull/76#discussion_r421571383
     *
     * This method has side effects. It creates a new Cursor to hold rows from new
     * pages.
     * Ideally fetching next set of rows using cursorId should be delegated to
     * Cursor.
    ...
    protected void buildNextPageFromCursorId() throws SQLException {
        try {
            ...
            JsonCursorHttpProtocol protocol = protocolFactory.getProtocol(connection.getConnectionConfig(),
                    (HttpTransport) connection.getTransport());
    ...
    protected <T> T getObjectX(int columnIndex, Class<T> javaClass, Map<String, Object> conversionParams)
            throws SQLException {
        final Object value = getColumn(columnIndex);
        // Change made to identify if value is of type array since it isn't an official
        // supported field type

        JDBCType trueJdbcType = getColumnMetaData(columnIndex).getInfinoType().getJdbcType();
        ...
        if (javaClass == Array.class) {
            ...
            conversionParams.put("baseType", getColumnMetaData(columnIndex).getInfinoType().getJdbcType());
        }
        ...
        if (null == tc) {
            throw new SQLException(
                    "Conversion from " + getColumnMetaData(columnIndex).getInfinoType() + " not supported.");
        }
    ...
    private Object getObjectX(int columnIndex, Map<String, Class<?>> map) throws SQLException {
        ...
        if (map != null) {
            columnSQLTypeName = getColumnMetaData(columnIndex).getInfinoType().getJdbcType().getName();
