...
1. This draft release triggers the [jenkins release workflow](https://build.ci.infino.ai/job/jdbc-release) as a result of which the driver is released on [maven central](https://search.maven.org/search?q=org.infino.driver). Please note that the release workflow is triggered only if created release is in draft state.
1. Once the above release workflow is successful, the drafted release on GitHub is published automatically.
1. Increment "version" in [build.gradle](./build.gradle) to the next iteration, e.g. 1.0.0.1 See [example](https://github.com/infinohq/jdbc/pull/11).